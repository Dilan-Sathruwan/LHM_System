...
if (isset($_POST['halls'])) {
    // Fetch the hall ID from the POST request
    $hall_id = $_POST['halls'];

    // Fetch the schedule for the selected hall
    $query = "
        SELECT 
        ...
        WHERE ls.hall_id = ?  -- Use hall_id for filtering
            ...
            <div class="container mt-2 bg-light p-4 rounded">
                <form action="" id="lForm" method="post">
                    <div class="row g-3">
                        <!-- Hall Selection -->
                        <div class="col-md-4">
                            <label for="inputHall" class="form-label fw-bold">Select Hall</label>
                            <select id="inputHall" class="form-select" name="halls" required>
                                <option selected value="">Choose a Hall</option>
                                <?php
                                $stmt = $conn->query("SELECT id, hall_name FROM lecture_halls");
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    echo "<option value='" . $row['id'] . "'>" . htmlspecialchars($row['hall_name']) . "</option>";
                                }
                                ?>
                            </select>
                        </div>
            ...
            <div class="container mt-5 bg-light p-4 rounded printLectures">
                <h3 class="text-start">Lecture Timetable for
                    ...
                    if (isset($_POST['halls'])) {
                        ...
                        $stmt = $conn->prepare("SELECT id, hall_name FROM lecture_halls WHERE id = :hall_id");
                        ...
                        if ($row) {
                            echo htmlspecialchars($row['hall_name']);
                        } else {
                            echo "Hall not found";
                        }
                    }
                ...
                <?php if (empty($lectures)) : ?>
                    <div class="alert alert-warning text-center">
                        <p>No scheduled lectures found for this hall.</p>
                    </div>
                <?php endif; ?>
            </div>
