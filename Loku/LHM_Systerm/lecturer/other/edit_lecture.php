...
if (isset($_GET['id'])) {
    ...
    if (!$lecture) {
        error_log("Lecture not found: Lecture ID " . $lecture_id . " for User ID " . $_SESSION['user_id']);
        die("Lecture not found.");
    }
} else {
    error_log("Invalid request: No lecture ID provided for User ID " . $_SESSION['user_id']);
    die("Invalid request.");
}
